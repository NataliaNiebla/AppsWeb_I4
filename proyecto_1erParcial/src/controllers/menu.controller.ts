...
export const getMenuItems = async (req: Request, res: Response) => {
    try {
        const items = await Menu.find({ isActive: true }).select('-__v');
        res.json(items);
    } catch (error) {
...
export const createMenuItem = async (req: Request, res: Response) => {
    ...
    const newItem = new Menu({ title, path, icon, roles, isActive: true });
    await newItem.save();
...
// PATCH: Baja lógica de un ítem del menú
    ...
    try {
        const { id } = req.params;
        const item = await Menu.findByIdAndUpdate(id, { isActive: false }, { new: true });
        if (!item) {
